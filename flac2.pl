...
sub flac2 {
    my ($out_format, $cue) = @_;	# 出力フォーマット, CUE シートファイル名
...
#	$pn = 9; for ($i = $pn; $i <= $track; $i++) {
    	...
    	$out = sprintf("%02d_%s.%s", $track_num[$i], &fname($track_title[$i]), $out_format);
    	$out = encode('utf8', $out);
    	...
    	#
    	# 切り出したWAVを $out_format に変換する
    	#
        if ($out_format eq 'mp3') {
        	@cmd = (
        	    "$LAME",
        	    "--cbr",
        	    "-b", "128",
